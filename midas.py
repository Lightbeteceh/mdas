...
class MidasApp:
    ...
    def handle_task(self, token, task):
        if 'completed' in task:
            if task['completed']:
                self.log(f"Task '{task['name']}' already completed.")
        ...
        if 'canBeClaimedAt' in task and task['canBeClaimedAt'] is not None:
            ...
            if claim_result['success']:
                self.log(f"Reward claimed for task '{task['name']}'")
            else:
        ...
        claim_result = self.claim_task(token, task['id'])
        if claim_result['success']:
            self.log(f"Task '{task['name']}' completed and reward claimed")
        else:
            ...
            try:
                ...
                try:
                    ...
                    if streak_response.status_code == 200:
                        ...
                        self.log(f"Successfully checked in on day {streak_days}: rewards {next_points} Points - {next_tickets} Tickets", Fore.GREEN)
                        ...
                        try:
                            ...
                            if task_result['success']:
                                if 'message' in task_result:
                                    ...
                                    self.log(f"Task '{task.get('name', 'Unknown task')}' completed", Fore.GREEN)
                            else:
                                self.log(f"Cannot complete task: {task.get('name', 'Unknown task')} (not eligible)", Fore.YELLOW)
                        except Exception as e:
                            self.log(f"Error processing task '{task.get('name', 'Unknown task')}': {str(e)}", Fore.RED)
                            self.log(f"Task details: {json.dumps(task, indent=2)}", Fore.RED)
                else:
                    self.log(f"Unable to fetch task list: {tasks_result['error']}", Fore.RED)
    ...
    def run_continuously(self):
        while True:
            self.log("This tool is shared on the Dân Cày Airdrop telegram channel (@dancayairdrop)", Fore.MAGENTA)
            self.process_accounts()
            self.log("Account processing cycle completed", Fore.MAGENTA)
            self.log("Waiting 24 hours before starting the next cycle...", Fore.YELLOW)
            time.sleep(86400)
